...
numstocsim = 21;
xs = zeros(numstocsim,3);
apogees = zeros(numstocsim);
OTRSs = zeros(numstocsim);
burntime= zeros(numstocsim);

for si = 1:1:numstocsim % First loop is mean
...
h = 2200; % elevation of launch site
if si==1
    Temp0 = 70;
...
dw = 95.496;
T0 = readmatrix('352022StaticFire.xlsx');
tstocbool = true;
if si ~= 1
    randthrust = 18.08+normrnd(0,1);
    if randthrust > 18.08
        tstocbool = false;
    end
    for thrustcounter = 1:length(T0) & tstocbool
        if T0(thrustcounter, 1) > randthrust
            T0 = T0(1:thrustcounter,:);
            tstocbool = false;
        end
...
Imp = trapz(T0(:,1),T0(:,2)); %calculate impulse using Riemanns
Impfactor = 9200/Imp-0.037;
%T0 = T0*Impfactor;
%Imp = trapz(T0(:,1),T0(:,2));
% Isp = 200;
wp = 58.8; %weight of propellant
OF = 34.339/24.461; %oxidizer to fuel ratio
wfi = wp/(OF+1);
woi = wfi*OF;
ww = dw+wfi+woi; %wet weight
Isp = Imp/(ww-dw); %specific impulse
lfi = 26.604/12; %length of fuel tank
loi = 26.604/12; %length of oxidizer tank
cgfi = 92/12 + 1/12 + loi + 0.5*lfi; %center of grav of fuel tank
cgoi = 92/12 + 0.5*loi; %center of grav of ox tank
xfuel0 = cgfi-lfi/2; %position of fuel tank wrt nose
xox0 = cgoi-loi/2; %position of ox tank wrt nose
dtank = 7/12; %diameter of tank
rtank = dtank/2; %radius of tank
% dnoz = 4;
dnoz = 1.5;  %diameter of nozzle
cgd = 93.546/12; %dry center of mass
if si==1 %non-stochastic
    Ecg = 1; %center of grav error
    RLa = 3*pi/180; %rail angle
else
    Ecg = normrnd(1,0.025^2);
    RLa = normrnd(3*pi/180,0.1^2);
end
RL = 55; %rail length
p0 = RLa; %initial direction
RLv = [-RL*sin(RLa),RL*cos(RLa)]; %rail vector
df = (wfi/g)/(lfi*pi*rtank^2); %density of fuel
dox = (woi/g)/(loi*pi*rtank^2); %density of oxidizer
ejection = 0;
...
%     end
    ...
    Cfg(i) = Cf(i,2)*(log10(Rn(i)))^2.6/(tr^2-1)*(tr^2*(log10(Rn(i)*tr))^(-2.6)-...
        ...
        (log10(Rn(i)))^(-3.6)));
    CDff(i) = Cfg(i)*(1+60*(Tf/Cr)^4+0.8*(1+5*(Cr*0.3/Cr)^2)*(Tf/Cr))*4*n*Afe2/pi/(d/12)^2;
