...
h = 4595; %altitude of launch site
x0 = 282; %x-position on map
y0 = 30.5; %y-position on map
Temp0 = 95;
kTemp = 10000; % Seems to be the most reasonable "tending to atmospheric model" rate
P0 = 14.24*144;
hr0 = 0; %humidity
%g = 32.174;
lat = 32*pi/180; %latitude
glat = 9.7803*((1+0.001932*(sin(lat))^2)/(sqrt(1-0.006694*(sin(lat))^2)));
%g = (glat-3.086*(10^(-6))*(h/3.28))*3.28;
GM = 1.4077*10^16; % Gravitational constant
re = 20.902*10^6; % Radius of Earth (ft)
g = GM/(re+h)^2; % gravitational acceleration

Eomega = 7.2921*10^(-5); %Coriolis Frequency

%vwgx = -5*5280/3600;
%vwgx = -27;
%vwg = 5;
vwgx = -15*5280/3600; %0.7157; % wind
vwgy = -15*5280/3600; %-7.2982; %
k = 0.4;
dref = 0.000;
z1 = 0.1/100;
atm_stability = 1; % 1=neutral, 2=stable, 3=unstable

dw = 68.24; %100.688; %106.5; 117.945; %dry weight
T0 = readmatrix('thrustArray6.xlsx');
% T0 = readmatrix('thrustArray.xlsx');
% T0 = readmatrix('Odyssey3Thrust.xlsx');
% T0 = readmatrix('PropSimThrust.xlsx');
%T0(:,2) = 0.992*T0(:,2);
Imp = trapz(T0(:,1),T0(:,2)); %impulse
%Impfactor = 9200/Imp-0.037;
%T0 = T0*Impfactor;
%Imp = trapz(T0(:,1),T0(:,2));
% Isp = 200;
wp = 29.781; %weight propellant
OF = 5.956; %ox-fuel ratio
wfi = wp/(OF+1); %initial weight of fuel
woi = wfi*OF; %initial weight of ox
ww = dw+wfi+woi; %wet weight
Isp = Imp/(ww-dw); %specific impulse
lfi = 38/12; %22.598/12; %22.25/12; %length of fuel grain 
loi = 57.606/12; %24.598/12; %25.07/12; %length of ox tank
cgfi = (56.041 + 87.959)/12; %131.525/12; %124.43/12; %cg of fuel
cgoi = (56.041 + 28.803)/12; %101.975/12; %96.18/12; %cg of ox
xfuel0 = cgfi-lfi/2; %initial position of fuel
xox0 = cgoi-loi/2; %initial position of ox
dtank = 7/12; %diameter of tank
rtank = dtank/2; %radius of tank
% dnoz = 4;
dnoz = 2.5; %2.81; %diameter of nozzle
cgd = 89.421/12; %101.28/12; %dry CG
RL = 345/12;  %taken from OpenRocket %34.4; %rail length
RLa = 3*pi/180; %rail angle %does not work for > 3 for some reason.
p0 = RLa;
RLv = [-RL*sin(RLa),RL*cos(RLa)]; %rail vector
df = (wfi/g)/(lfi*pi*rtank^2); %density of fuel
dox = (woi/g)/(loi*pi*rtank^2); %density of ox
ejection = 0;

ti = 0;
tb = T0(end,1); %time of burn
% tb = round(find(T0(:,2),1,'last')/1000,2); %PropSim
tf = 500; %time of flight
dt = 0.01; %time step
t_steps = tf/dt; %number of time steps

Tt = interp1(T0(:,1),T0(:,2),ti:dt:tb); %thrust interpolation
% if dt==0.001
...
% ma = mach_cd(:,1);
% cdv = mach_cd(:,6);
vs = zeros(t_steps,1); %velocities
gamma = 1.4; %specific heat ratio of air
R = 1717; %gas constant for air
mach = zeros(t_steps,1); %mach number
cd8 = zeros(t_steps,1); %coeff of drag
cl8 = zeros(t_steps,1); %coeff of lift

d = 5.114; %diameter of rocket
Aref = pi*(d/12/2)^2;
%% Fin Vars
n = 4; % number of fins
CNan = 2; % normal force coeff of nose cone (coeff of lift)
dr = 4.35/12; %diameter of
DR = dr/2;
CNat = 2*((dr/(d/12))^2-1); %normal force coeff of boattail
r0 = d/2;
S = 5/12; %span
% S = 1/12;
Cr = 5/12; % root chord length
Ct = 5.01/12; % tip chord length (5.01 cuz div by 0 error otherwise)
Lf = sqrt(S^2+(Cr/2-Ct/2)^2); %length of fin mid-chord line
dfa = dr;
Kf = 1+(dr/2)/(S+(dr/2)); 
CNaf = Kf*((4*n*(S/(d/12))^2)/(1+sqrt(1+(2*Lf/(Cr+Ct))^2)));
Ln = 20.4/12; %length of nosecone
xn = 0.466*Ln; %center of mass of nosecone
LB = (25.3+12.61+51.11+35.125)/12; %length of body tube
xp = Ln+LB; %xposition of boattail
Lt = 6.65/12; %15.75/12;
xt = xp+Lt/3*((1+(1-(d/12)/dr)/(1-((d/12)/dr)^2)));
Rl = Ln+LB+Lt; %rocket length
xB = Rl-Cr; %distance from nose tip to fin root chord leading edge
xr = Cr-Ct; %distance between fin root leading edge and fin tip leading edge parallel to body
xf = xB+xr/3*(Cr+2*Ct)/(Cr+Ct)+(1/6*((Cr+Ct)-(Cr*Ct)/(Cr+Ct))); %x-position of body
xBody = Ln+0.5*LB; %position of middle of vody tube
K = 1.5;
ApB = (d/12)*LB;
Apb = Lt/2*((d/12)+dr);
...
Ap = ApB+Apb+Apn;
CNab = 0;
Tf = 0.6/12; %root thickness
thetaLE = atan(xr/S);
thetaTE = atan((Cr-xr-Ct)/S);
Afe2 = 2*(Cr*S-0.5*S^2*(tan(thetaLE)+tan(thetaTE)))+Tf*S*0.3;
Afe = S*(Cr+Ct); %total fin exposed area
Afp = Afe+0.5*(1/12)*Cr; %total fin planform area
Rs = (2*S+d/12)/dfa;
kfb = 0.8065*Rs^2+1.1553*Rs; %fin body interference coeff
kbf = 0.1935*Rs^2+0.8174*Rs+1; %body fin interference coeff
tr = Ct/Cr;
...
%% Fairing Variables
rpro = 1.35/12; %radius
Apro = pi/2*rpro^2; %Projected Area
Lp = 55.74/12; %Length of Protuberance
aPro = 76.78; %Distance from nosecone to Protuberance
Spro = 2*2*Apro+2*pi*rpro*Lp; %Surface Area of protuberance
...
% tmap = 0.0;
tmap = 0;
tmam = 0.32/180*pi; %thrust misalignment mag
%tmam = 0;
...
RM(1:2,:) = -RM(1:2,:);
RMrec{1} = RM;
yA(:,1) = RM*[1;0;0]; % yaw axis
pA(:,1) = RM*[0;1;0]; % pitch axis
rA(:,1) = RM*[0;0;1]; %roll axis
% rA(1,1) = -rA(1,1);
tA(:,1) = [cos(tma(2)),-sin(tma(2)),0;sin(tma(2)),cos(tma(2)),0;0,0,1]*... %thrust axis
