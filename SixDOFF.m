...
%     vrwrot(i,:) = ((vrotx(i,:)+vrw(i,1)).^2+(vrotz(i,:)+vrw(i,2)).^2).^0.5;
    ...
    getCD0(temp(i),mu0,temp0,rho(i),vrwrot(i,:),vrw(i,:),mach(i),Lc,tr,Tf,Cr,Ct,S,d,Rl,n,...
...
%             disp(i+1)
        ...
        if zs(i) > 900
            cd8(i) = 2.2; %drag coeff
            d = 36; % drogue diameter
            Aref = pi*(d/12/2)^2;
%             D(i,:) = cd8(i)/2*Aref*rho(i)*(norm(vrw(i,:)))^2*[sin(phi(i)),-cos(phi(i))];
            D(i,:) = -cd8(i)/2*Aref*rho(i)*(norm(vrw(i,:)))^2*vhat(i,:);
...
%     elseif ((norm(T(i,:))-w(i)*cos(p(i)))<=abs(Frfs(i)) && dist(i) <= 0) && t<tb
    elseif i <= tb*100 && ((norm(T(i,:))-w(i)*cos(p(i)))<=abs(Frfs(i)) && dist(i) <= 0) && t<tb 
...
m1 = 7.553; %9.62; %Nose
m2 = 7.616; %28.48+2.5; %Recov Bay
m3 = 8.449; %7.390+wo(I_maxQCNa); %8.87 %EBay
m4 = 9.639; %2.26; %Pressurant Tank
m5 = 11.121; %7.390+wf(I_maxQCNa)+1.13; %2.5+8.12 %PlumBay
m6 = 5.6+wo(I_maxQCNa); %24.325+16.280; %9.35; %LOx Tank
m7 = 6.6; %19.39; %Intertank
m8 = 5.6+wf(I_maxQCNa); %Fuel Tank
m9 = 11.566; %Engine Bay
m10 = 23.420; %Engine + BT
masses = [m1,m2,m3,m4,m5,m6,m7,m8,m9,m10]/gs(I_maxQCNa);
distances = [0,35,10,10,18,17,loi*12,1,lfi*12,16.875,19.70,0];
% m1 = 9.62; %Nose
% m2 = 28.48+2.5;
% m3 = 8.87+wo(1);
% m4 = 2.26;
...
%%
figure(1)
plot(ts(1:itf),zs(1:itf)); hold on;
yline(10000, 'color', 'g');
%yline(zs(round(tb/dt)),'color', 'g'); % make into integer
%xline(tb, 'color', 'r');
xlabel('Time (s)')
ylabel('Altitude (ft)')
legend('Altitude', 'Target Apogee (10k ft)')
...
% plot(ts(1:itf),v(1:itf,3)/1125.3);
% xlabel('Time')
% ylabel('Vertical Velocity (Mach Number)')
%%
% figure
...
legend('Angle of attack','Launch Rail Clearance', 'Motor Burnout')
...
legend('Stability','Launch Rail Clearance', 'Motor Burnout')
title('Stability vs Time')
% %%
% figure
